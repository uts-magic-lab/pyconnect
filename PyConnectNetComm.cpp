...
PyConnectNetComm::PyConnectNetComm()
    : ObjectComm(), pMP_(NULL), udpSocket_(INVALID_SOCKET),
      tcpSocket_(INVALID_SOCKET), domainSocket_(INVALID_SOCKET),
      dgramBuffer_(NULL), clientDataBuffer_(NULL), dispatchDataBuffer_(NULL),
      clientFDList_(NULL), maxFD_(0), netCommEnabled_(false),
      IPCCommEnabled_(false), invalidUDPSock_(false), keepRunning_(true),
...
bool PyConnectNetComm::initUDPListener() {
  if ((udpSocket_ = socket(AF_INET, SOCK_DGRAM, 0)) == INVALID_SOCKET) {
    ERROR_MSG(
        "PyConnectNetComm::initUDPListener: unable to create UDP socket.\n");
...
#ifdef USE_MULTICAST
  char ttl = 30;
  if (setsockopt(udpSocket_, IPPROTO_IP, IP_MULTICAST_TTL, (char *)&ttl,
                 sizeof(ttl)) < 0) {
    ERROR_MSG("PyConnectNetComm::initUDPListener: failed to set multicast TTL "
              "on UDP socket.\n");
#ifdef WIN32
  ...
  if (setsockopt(udpSocket_, IPPROTO_IP, IP_ADD_MEMBERSHIP,
                 (char *)&multiCastReq_, sizeof(multiCastReq_)) < 0) {
    ERROR_MSG("PyConnectNetComm::initUDPListener: failed to join multicast "
              "group on UDP socket.\n");
#ifdef WIN32
    closesocket(udpSocket_);
#else
...
#else // !USE_MULTICAST
  if (setsockopt(udpSocket_, SOL_SOCKET, SO_BROADCAST, (char *)&turnon,
                 sizeof(turnon)) < 0) {
    ERROR_MSG("PyConnectNetComm::initUDPListener: failed to enable broadcast "
              "on UDP socket.\n");
#ifdef WIN32
    closesocket(udpSocket_);
#else
  ...
  if (setsockopt(udpSocket_, SOL_SOCKET, SO_REUSEADDR, (char *)&turnon,
                 sizeof(turnon)) < 0) {
    ERROR_MSG("PyConnectNetComm::initUDPListener: failed to enable reuse "
              "address on UDP socket.\n");
#ifdef WIN32
    closesocket(udpSocket_);
#else
    close(udpSocket_);
  ...
  if (bind(udpSocket_, (struct sockaddr *)&sAddr_, sizeof(sAddr_)) < 0) {
    ERROR_MSG("PyConnectNetComm::initUDPListener: unable to bind to network "
              "interface.\n");
#ifdef WIN32
    closesocket(udpSocket_);
#else
...
bool PyConnectNetComm::initTCPListener() {
  if ((tcpSocket_ = socket(AF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) {
    ERROR_MSG(
        "PyConnectNetComm::initTCPListener: unable to create TCP socket.\n");
    return false;
  }

#ifndef WIN32
  int turnon = 1;
  if (setsockopt(tcpSocket_, SOL_SOCKET, SO_REUSEADDR, (char *)&turnon,
                 sizeof(turnon)) < 0) {
    ERROR_MSG("PyConnectNetComm::initTCPListener: failed to enable reuse addr "
              "option on TCP socket.\n");
    close(tcpSocket_);
    return false;
  }
#endif
  ...
  if (bindRet < 0) {
    ERROR_MSG("PyConnectNetComm::initTCPListener: unable to bind to network "
              "interface.\n");
#ifdef WIN32
  ...
  if (listen(tcpSocket_, 5) < 0) {
    ERROR_MSG("PyConnectNetComm::initTCPListener: unable to listen for "
              "incoming data.\n");
#ifdef WIN32
    closesocket(tcpSocket_);
#else
...
void PyConnectNetComm::processIncomingData(fd_set *readyFDSet) {
  ...
  if (netCommEnabled_) {
    if (FD_ISSET(udpSocket_, readyFDSet)) {
      int readLen = (int)recvfrom(udpSocket_, (char *)dgramBuffer_,
                                  PYCONNECT_UDP_BUFFER_SIZE, 0,
                                  (sockaddr *)&cAddr, (socklen_t *)&cLen);
      if (readLen <= 0) {
  ...
  while (FDPtr) {
    ...
    if (FD_ISSET(fd, readyFDSet)) {
      ...
      if (readLen <= 0) {
        ...
        do {
          if (*dataPtr == PYCONNECT_MSG_INIT &&
              FDPtr->dataInfo.expectedDataLength == 0) { // new message
            if (readLen < 4) {
              ERROR_MSG("PyConnectNetComm::continuousProcessing: "
            ...
            if (dataCount < 0 ||
                dataCount >
                    PYCONNECT_MSG_BUFFER_SIZE) { // encryption buffer size
              ERROR_MSG("PyConnectNetComm::continuousProcessing: "
                        "invalid data size in stream on %d.\n",
                        fd);
              break;
            } else if (dataCount >
                       (readLen - 1)) { // the message needs multiple reads
              FDPtr->dataInfo.bufferedDataLength = readLen;
              memcpy(FDPtr->dataInfo.bufferedData, dataPtr,
                     FDPtr->dataInfo.bufferedDataLength);
              FDPtr->dataInfo.expectedDataLength =
                  dataCount - FDPtr->dataInfo.bufferedDataLength;
              readLen = 0;
            } else if (*(dataPtr + dataCount) ==
                       PYCONNECT_MSG_END) { // valid message
          ...
          } else if (FDPtr->dataInfo.expectedDataLength >
                     0) { // patch up cached data
            unsigned char *cachedPtr = FDPtr->dataInfo.bufferedData +
                                       FDPtr->dataInfo.bufferedDataLength;
            if (FDPtr->dataInfo.expectedDataLength >
                (readLen - 1)) { // continue to cache
              memcpy(cachedPtr, dataPtr, readLen);
              FDPtr->dataInfo.bufferedDataLength += readLen;
              FDPtr->dataInfo.expectedDataLength -= readLen;
              readLen = 0;
            } else if (*(dataPtr + FDPtr->dataInfo.expectedDataLength) ==
                       PYCONNECT_MSG_END) { // valid message
...
void PyConnectNetComm::dataPacketSender(const unsigned char *data, int size,
                                        bool broadcast) {
  if (broadcast) {
...
void PyConnectNetComm::netBroadcastSend(const unsigned char *data, int size) {
  ...
  if (netCommEnabled_) {
    if (sendto(udpSocket_, (char *)outputData, outputLength, 0,
               (struct sockaddr *)&bcAddr_, sizeof(bcAddr_)) < 0) {
      ERROR_MSG("PyConnectNetComm::broadcastSend: Error sending UDP broadcast "
                "packet.\n");
    }
  }
...
void PyConnectNetComm::processUDPInput(unsigned char *recBuffer, int recBytes,
                                       struct sockaddr_in &cAddr) {
  unsigned char *message = NULL;
  int messageSize = 0;

  if (decryptMessage(recBuffer, (int)recBytes, &message, (int *)&messageSize) !=
      1) {
...
#endif
  {
    ...
    if (createTCPTalker(cAddr)) { // process udp data
      MesgProcessResult procResult =
          pMP_->processInput(message, messageSize - sizeof(short), cAddr, true);
      if (procResult == MESG_TO_SHUTDOWN) {
...
SOCKET_T PyConnectNetComm::findOrCreateIPCTalker(int procID) {
  ...
  if (mySocket != INVALID_SOCKET) {
    ...
    int connLen =
        int(offsetof(struct sockaddr_un, sun_path) + strlen(dAddr.sun_path));
...
SOCKET_T
PyConnectNetComm::findFdFromClientListByProcID(int procID) // for IPC only
{
  bool found = false;
...
void PyConnectNetComm::consolidateIPCSockets() {
  ...
  while ((dp = readdir(socketDir)) != NULL) {
    if (fnmatch(socketPattern, dp->d_name, 0) == 0) {
      ...
      int procID =
          (int)strtol(strlen(PYCONNECT_CLTSOCKET_PREFIX) + 1 + dp->d_name,
                      (char **)NULL, 10);
      if (procID == 0) {
      ...
      kernQuery[3] = procID;
      foundProc =
          ((sysctl(kernQuery, 4, &kp, &kplen, NULL, 0) == 0) && kplen > 0);
...
#ifdef USE_MULTICAST
  setsockopt(udpSocket_, IPPROTO_IP, IP_DROP_MEMBERSHIP, (char *)&multiCastReq_,
             sizeof(multiCastReq_));
#endif
#ifdef WIN32
...
void PyConnectNetComm::enableIPCComm() {
  ...
  if ((domainSocket_ = socket(AF_UNIX, SOCK_STREAM, 0)) == INVALID_SOCKET) {
    ERROR_MSG("PyConnectNetComm::enableIPCComm: unable to create unix domain "
              "socket.\n");
    return;
  }
  ...
  if (bind(domainSocket_, (struct sockaddr *)&dAddr, bindLen) < 0) {
    ERROR_MSG("PyConnectNetComm::enableIPCComm: unable to bind to local domain "
              "socket.\n");
#ifdef WIN32
#else
    close(domainSocket_);
  ...
  if (listen(domainSocket_, 5) < 0) {
    ERROR_MSG("PyConnectNetComm::enableIPCComm: unable to listen on local "
...
void PyConnectNetComm::disableIPCComm(bool onExit) {
  ...
  // Assertion failed: (autoInterpreterState), function PyGILState_Ensure, file
  // Python/pystate.c, line 563
  ClientFD *fdPtr = clientFDList_;
  while (fdPtr) {
    if (fdPtr->domain != PyConnectNetComm::LOCALIPC) {
      fdPtr = fdPtr->pNext;
...
CommObjectStat PyConnectNetComm::setBroadcastAddr(char *addr) {
  ...
  if (saddr == INADDR_NONE && strcmp(addr, "255.255.255.255")) {
#else
...
CommObjectStat PyConnectNetComm::createTalker(char *host, int port) {
  ...
  if (port == 0)
    port = PYCONNECT_NETCOMM_PORT;
           ...
           port > PYCONNECT_NETCOMM_PORT + PYCONNECT_COMMPORT_RANGE)
    return INVALID_ADDR_PORT;
...
void PyConnectNetComm::addFdToClientList(const SOCKET_T &fd, FDDomain domain,
                                         struct sockaddr_in *cAddr,
                                         int procID) {
#ifdef MULTI_THREAD
#ifdef WIN32
  ...
  if (domain == PyConnectNetComm::NETWORK) {
    int turnon = 1;
    if (setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, (char *)&turnon, sizeof(int)) <
        0) {
      ERROR_MSG("Unable to set TCP_NODELAY to socket %d\n", fd);
    }
    turnon = kTCPMSS;
    if (setsockopt(fd, IPPROTO_TCP, TCP_MAXSEG, (char *)&turnon, sizeof(int)) <
        0) {
...
void PyConnectNetComm::destroyCurrentClient(ClientFD *&FDPtr,
                                            ClientFD *&prevFDPtr) {
...
bool PyConnectNetComm::getIDFromIP(int &addr) {
  ...
  int rc = WSAIoctl(mySock, SIO_ADDRESS_LIST_QUERY, NULL, 0, NULL, 0,
                    &dwBytesRet, NULL, NULL);

  if (rc == SOCKET_ERROR &&
      GetLastError() == WSAEFAULT) { // retrieve output buffer size
    ...
    rc = WSAIoctl(mySock, SIO_ADDRESS_LIST_QUERY, NULL, 0, buf, dwBytesRet,
                  &dwBytesRet, NULL, NULL);
    if (rc != SOCKET_ERROR) {
...
bool PyConnectNetComm::getIDFromIP(int &addr) {
  ...
  for (; (char *)lifr < (char *)mylifconf.lifc_req + mylifconf.lifc_len;
       ++lifr) {
    ...
    if ((lifr->lifr_flags & IFF_UP) &&
        !(lifr->lifr_flags &
          IFF_LOOPBACK)) { // found an live non-loopback interface
