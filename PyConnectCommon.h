...
#define PYCONNECT_LOGGING_INIT s_pyconnectlog = fopen(logFileName, "a")
...
#pragma message(                                                               \
    "Make sure you change the encryption key for network communication. \
...
struct PyConnectType {
  ...
  static PyObject *unpackStr(Type type, unsigned char *&dataPtr,
                             int &remainingLength);
#endif
};
...
template <> struct pyconnect_type<void> {
  static constexpr PyConnectType::Type value = PyConnectType::PyVOID;
};
template <> struct pyconnect_type<bool> {
  static constexpr PyConnectType::Type value = PyConnectType::BOOL;
};
template <> struct pyconnect_type<int> {
  static constexpr PyConnectType::Type value = PyConnectType::INT;
};
template <> struct pyconnect_type<float> {
  static constexpr PyConnectType::Type value = PyConnectType::FLOAT;
};
template <> struct pyconnect_type<double> {
  static constexpr PyConnectType::Type value = PyConnectType::DOUBLE;
};
template <> struct pyconnect_type<std::string> {
  static constexpr PyConnectType::Type value = PyConnectType::STRING;
};

template <typename T>
static const PyConnectType::Type getVarType(const T &val) {
#ifndef _MSC_VER
  int status = 0;
...
#endif
  ERROR_MSG("PyConnect::getVarType: c++ data type %s is not supported",
            demangled);
...
template <typename T> static const char *getVarTypeName(const T &val) {
  ...
  char *demangled = typeid(val).name();
#endif
  ERROR_MSG("PyConnect::getVarTypeName: c++ data type %s is not supported",
            demangled);
  return "";
}
...
template <> const char *getVarTypeName(const int &) { return "integer"; }
template <> const char *getVarTypeName(const float &) { return "float"; }
template <> const char *getVarTypeName(const double &) { return "double"; }
template <> const char *getVarTypeName(const bool &) { return "boolean"; }
template <> const char *getVarTypeName(const std::string &) { return "string"; }
...
struct function_traits<std::function<R (*)(Args...)>>
    : public function_traits<std::function<R(Args...)>> {};
...
auto get_arg_types(std::vector<int> &atlist) ->
  ...
  atlist.push_back(pyconnect::pyconnect_type<typename std::decay<
                       typename fun::template argument<0>::type>::type>::value);
}
...
auto get_arg_types(std::vector<int> &atlist) ->
  ...
  using fun = function_traits<std::function<R(Args...)>>;
  atlist.push_back(
      pyconnect::pyconnect_type<typename std::decay<
          typename fun::template argument<ArgIdx - 1>::type>::type>::value);

  get_arg_types<ArgIdx - 1, R, Args...>(atlist);
}

template <typename F, typename std::enable_if<(F::arity > 0), int>::type = 0,
          std::size_t... Is>
void get_args_type_list(std::index_sequence<Is...>, std::vector<int> &rtl) {
  get_arg_types<F::arity, typename F::return_type,
                typename F::template argument<Is>::type...>(rtl);
}
...
static int packToLENumber(const DataType &v, unsigned char *&dataPtr) {
...
template <typename DataType>
static void unpackLENumber(DataType &val, unsigned char *&dataPtr,
                           int &remainingLength) {
  // TODO: optimise this operation
  int dataLength = sizeof(DataType);
#ifdef WITH_BIG_ENDIAN
